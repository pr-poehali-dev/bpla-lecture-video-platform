...
const TACMED_CATEGORIES = ["Первая помощь", "Турникеты и жгуты", "Эвакуация", "Протоколы", "Аптечка"];

const SECTIONS = [
  { key: "general", label: "Материалы / Видео" },
  { key: "tacmed", label: "Так Мед" },
...
export default function AdminFilesTab({ isAdmin = false }: Props) {
      ...
      fetch("https://functions.poehali.dev/0edb3a50-4c27-43a5-b907-883104f0c559/", {
        ...
        .then((res) => {
          ...
          if (res.id) {
            ...
            loadFiles();
            const uploadedSection = form.section;
            const linkPage = uploadedSection === "tacmed" ? "tacmed" : uploadedMime.startsWith("video/") ? "videos" : "lectures";
            api.notif.adminSend({ title: `Новый материал: ${uploadedTitle}`, type: "new_content", link_page: linkPage }).catch(() => {});
          } else {
  ...
  const isTacmed = form.section === "tacmed";
