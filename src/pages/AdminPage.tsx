...
function FileUploadTab() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ACCEPTED_MIME[f.type]) {
      showMsg(`Формат не поддерживается: ${f.type}`, false);
      return;
    }
    setSelectedFile(f);
  ...
  const categories = detectedType === "video" ? VIDEO_CATEGORIES : DOC_CATEGORIES;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 space-y-4" style={{ border: "1px solid #1a2a3a", background: "#0a1520" }}>
        <div className="font-mono text-xs text-[#00f5ff] tracking-wider mb-2">ЗАГРУЗИТЬ ФАЙЛ</div>
        ...
        {filesLoading ? (
          ...
          uploadedFiles.map((f, i) => (
            <div key={f.id} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: i < uploadedFiles.length - 1 ? "1px solid #0d1a28" : "none" }}>
              <Icon name={f.file_type === "video" ? "Video" : "FileText"} size={16} className={f.file_type === "video" ? "text-[#00f5ff]" : "text-[#ff6b00]"} />
              ...
              <span className="font-mono text-xs text-[#3a5570]">{f.file_type === "video" ? "ВИДЕО" : "ДОКУМЕНТ"}</span>
